...
export default function VFCRegistration() {
  ...
  const { admin_id, name , email, password, address, phone, reg_no , city,state,zip} = vfc;
  ...
  const onSubmit = async e => {
    e.preventDefault();  
    await axios.post("http://localhost:3001/VFRegistration", vfc);
    alert("data submited");
 ...
 <Typography style={{fontFamily:"Carter One",color:"#fff",fontSize:40,marginLeft:20}} >
  ...
  {/* <Form.Group as={Col} controlId="vfc_id">
    ...
    <Form.Group as={Col} controlId="admin_id">
      ...
      <Form.Control type="text" name="admin_id"  value={sessionStorage.admin_id} defaultValue={sessionStorage.admin_id} disabled   />
    </Form.Group>
  </Form.Row>
<Form.Row>
